...
export default function DeleteAcquiredAbilityModal(props: DeleteAcquiredAbilityModalProps) {
  const handleDelete = () => {
    deleteAcquiredAbility(props.acquiredAbility.id)
      .then(success => {
        if (success === true) {
          props.setOpen(false);
          props.onDelete();
      ...
      <Fade in={props.open}>
        <Box sx={{
          ...
          <Typography id="modal-modal-description" sx={{ mt: 2 }}>
            Вы точно хотите удалить приобретённую способность {displayName(props.acquiredAbility)}?
          </Typography>

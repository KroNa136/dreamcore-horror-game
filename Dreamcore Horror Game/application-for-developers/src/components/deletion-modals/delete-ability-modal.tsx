...
import { deleteAbility } from '../../requests';
...
export default function DeleteAbilityModal(props: DeleteAbilityModalProps) {
  const handleDelete = () => {
    deleteAbility(props.ability.id)
      .then(success => {
        if (success === true) {
          props.setOpen(false);
          props.onDelete();
      ...
      <Fade in={props.open}>
        <Box sx={{
          ...
          <Typography id="modal-modal-description" sx={{ mt: 2 }}>
            Вы точно хотите удалить способность {displayName(props.ability)}?
          </Typography>

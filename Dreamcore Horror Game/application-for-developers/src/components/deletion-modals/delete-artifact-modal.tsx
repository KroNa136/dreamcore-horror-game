...
import { deleteArtifact } from "../../requests";
...
export default function DeleteArtifactModal(props: DeleteArtifactModalProps) {
  const handleDelete = () => {
    deleteArtifact(props.artifact.id)
      .then(success => {
        if (success === true) {
          props.setOpen(false);
          props.onDelete();
      ...
      <Fade in={props.open}>
        <Box sx={{
          ...
          <Typography id="modal-modal-description" sx={{ mt: 2 }}>
            Вы точно хотите удалить артефакт {displayName(props.artifact)}?
          </Typography>
